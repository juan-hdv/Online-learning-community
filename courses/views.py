    ...
    if request.method == "POST":
        ...
        if usr is not None:
            ...
            return render(
                request,
                "courses/login.html",
                {"message": "Invalid credentials.", "msgType": "alert-danger"},
            )
    else:
    ...
    if request.method == "POST":
        ...
        if password != confirmation:
            return render(
                ...
                {"message": "Passwords must match.", "msgType": "alert-warning"},
            )
        ...
        try:
            ...
            return render(
                request,
                "courses/register.html",
                {"message": "Username already taken.", "msgType": "alert-warning"},
            )
...
def courseReview(request, idcourse=None):
    ...
    if request.method == "GET":
        ...
        courseReviews = CourseReviews.objects.exclude(user=usrCurrent).filter(
            course=course
        )
        return render(
            request,
        ...
        if not courseId:
            return render(
                request,
                "courses/error.html",
                {"msgType": "alert-danger", "message": "No id_courses"},
            )
        ...
        if action in "update":
            ...
            try:
                ...
                return render(
                    request,
                    "courses/register.html",
                    {"message": "Integrity error.", "msgType": "alert-warning"},
                )
        else:
            return render(
                request,
                "courses/error.html",
                {
                    ...
                    "message": f"Invalid operation - Unknown botton action: {action}",
                },
            )
        # Recalculate course average rating
        course.avgRating = float(
            CourseReviews.objects.filter(course=course).aggregate(Avg("rating"))[
                "rating__avg"
            ]
        )
        course.save()
        ...
        return render(
            request,
            "courses/error.html",
            {"msgType": "alert-danger", "message": "Bad request!"},
        )
...
def addToCart(request):
    if request.method != "POST":
        return render(
            request,
            "courses/error.html",
            {"msgType": "alert-danger", "message": "Only POST request allowed"},
        )
    ...
    try:
        ...
        return render(
            request,
            "courses/error.html",
            {"msgType": "alert-danger", "message": "Key error."},
        )
    except course.DoesNotExist:
        return render(
            request,
            "courses/error.html",
            {"msgType": "alert-danger", "message": "Course do not exist."},
        )
    ...
    for a in addsList:
        ...
        try:
            ...
            return render(
                request,
                "courses/error.html",
                {"msgType": "alert-danger", "message": "CourseAdd Key error."},
            )
        except CourseAdd.DoesNotExist:
            return render(
                request,
                "courses/error.html",
                {"msgType": "alert-danger", "message": "CourseAdd do not exist."},
            )
...
def deleteFromCart(request):
    ...
    if not orderId:
        return render(
            request,
            "courses/error.html",
            {"msgType": "alert-danger", "message": "No id_order"},
        )
...
def showCart(request):
    if request.method != "GET":
        return render(
            request,
            "courses/error.html",
            {"msgType": "alert-danger", "message": "Only GET request allowed"},
        )
    ...
    try:
        ...
        return render(
            request,
            "courses/error.html",
            {"msgType": "alert-danger", "message": "User do not exist."},
        )
...
def createPaypalOrder(request):
    if not (request.is_ajax and request.method == "POST"):
        return render(
            request, "courses/error.html", {"message": "Operation not allowed."}
        )
    ...
    try:
        ...
        return render(
            request,
            "courses/error.html",
            {"msgType": "alert-danger", "message": "Order do not exist."},
        )
    ...
    if response.status_code != 200:
        return render(
            ...
            {
                "msgType": "alert-danger",
                "message": f"HTTP Error {response.status_code} <getting token>.",
            },
        )
    ...
    response = paypal_createOrder(
        settings.PAYPAL_URLORDER, paypalOrder["token"], paypalOrder["details"]
    )
    if response.status_code != 201:  # Created
        return render(
            ...
            {
                "msgType": "alert-danger",
                "message": f"HTTP Error {response.status_code} <creating order>. Please try later.",
            },
        )
...
def capturePaypalOrder(request):
    ...
    if not paypalOrder:
        return render(
            ...
            {
                "msgType": "alert-danger",
                "message": "Payment failed. Nonexistent <token> or <order>.  Please restart your payment.",
            },
        )
    ...
    url = next(
        (item for item in paypalOrder["response"]["links"] if item["rel"] == "capture"),
        None,
    )["href"]
...
def cancelPaypalPayment(request):
    return render(
        ...
        "courses/information.html",
        {
            ...
            "message": f"Your payment has been successfully canceled. No charges have been applied.",
        },
    )
